...
class HumanoidWholeBody(BaseTask):
    ...
    def _setup_character_props(self, key_bodies):
        ...
        if (asset_file == "mjcf/smplx_capsule.xml"):
            ...
            self._num_obs = 1 + (53) * (3 + 6 + 3 + 3) - 3 + 10*3
        ######### parahome #########
        elif (asset_file in ["mjcf/mocap_parahome_boxhand.xml", "mjcf/mocap_parahome_boxhand_s22.xml"] or asset_file.startswith("mjcf/parahome/s")):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        ######### Modified by Runyi #########
        if "parahome" in self.cfg["env"]["asset"]["assetFileName"]:
            # Check for specific multi-object combinations
            static_objs = set(self.cfg['env']['in_scene_obj_static'])
            dynamic_obj = self.cfg['env']['in_scene_obj_dynamic'][0] if self.cfg['env']['in_scene_obj_dynamic'] else None

            # Kettle with desk+diningtable needs more space (high convex hull count)
            if (dynamic_obj == 'kettle' and
                'desk' in static_objs and 'diningtable' in static_objs):
                max_agg_bodies += 300
                max_agg_shapes += 300
            # Kettle or chair with any static objects
            elif dynamic_obj in ['kettle', 'chair']:
                max_agg_bodies += 300
                max_agg_shapes += 300
            # Sink in scene needs extra space
            elif len(self.cfg['env']['in_scene_obj_dynamic']) == 1 and 'sink' in static_objs:
                max_agg_bodies += 300
                max_agg_shapes += 300
    ...
    def _init_camera(self):
        ...
        cam_target = gymapi.Vec3(self._cam_prev_char_pos[0],
                                 ...
                                 1.0)
        # new_cam_pos = gymapi.Vec3(0.1, -0.8 , 0.90)
        # new_cam_target = gymapi.Vec3(-0.5, 0.5, 0.90)

        # new_cam_pos = gymapi.Vec3(-1, -1.5 , 1.2)
        # new_cam_target = gymapi.Vec3(-0.5, -1.5, 1.2)

        # new_cam_pos = gymapi.Vec3(1.5, -1 , 1.2)
        # new_cam_target = gymapi.Vec3(1.5, 0, 1.2)

        new_cam_pos = gymapi.Vec3(0, -2.0 , 1.2)
        new_cam_target = gymapi.Vec3(0, 0, 1.2)
