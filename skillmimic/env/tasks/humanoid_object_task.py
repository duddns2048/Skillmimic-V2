...
class HumanoidWholeBodyWithObjectParahome(HumanoidWholeBodyWithObject): #metric
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
        # Extract scene number from motion_file or assetFileName
        self.scene_number = self._extract_scene_number(cfg)
    ...
    def _extract_scene_number(self, cfg):
        """Extract scene number from motion_file or assetFileName.

        Examples:
            motion_file: "skillmimic/data/motions/ParaHome/wristmimic/s117_move_pan" -> 117
            assetFileName: "mjcf/parahome/s117.xml" -> 117
        """
        import re

        # Try to extract from motion_file first
        if 'motion_file' in cfg['env']:
            motion_file = cfg['env']['motion_file']
            # Match pattern like s117, s110, s22, s6, etc.
            match = re.search(r'/s(\d+)', motion_file)
            if match:
                return match.group(1)

        # Fall back to assetFileName
        if 'assetFileName' in cfg['env']['asset']:
            asset_file = cfg['env']['asset']['assetFileName']
            # Match pattern like s117.xml
            match = re.search(r's(\d+)', asset_file)
            if match:
                return match.group(1)

        # Default fallback (should rarely happen)
        print("Warning: Could not extract scene number from config, defaulting to s110")
        return "110"
    ...
    def _set_static_objects(self, asset_root, obj_name):
        if '_plane' in obj_name :
            ...
            static_asset_options.angular_damping = 0.01
            static_asset_options.linear_damping = 0.01
            static_asset_options.density = 200
            static_asset_options.default_dof_drive_mode = gymapi.DOF_MODE_NONE
            static_asset_options.vhacd_enabled = True
            static_asset_options.vhacd_params.resolution = 300000
            static_asset_options.vhacd_params.max_num_vertices_per_ch = 64

            # Set max_convex_hulls based on object type (matching InterMimic)
            static_asset_options.vhacd_params.max_convex_hulls = 10  # Default
            if obj_name in ['kettle', 'cup']:
                static_asset_options.vhacd_params.max_convex_hulls = 80
            elif obj_name == 'chair':
                static_asset_options.vhacd_params.max_convex_hulls = 50
            elif obj_name == 'bookshelf':
                static_asset_options.vhacd_params.max_convex_hulls = 150
    ...
    def _set_dynamic_objects(self, asset_root, obj_name):
        ...
        dynamic_asset_options.density = 200  # Match InterMimic density
        dynamic_asset_options.default_dof_drive_mode = gymapi.DOF_MODE_NONE
        dynamic_asset_options.vhacd_enabled = True
        dynamic_asset_options.vhacd_params.resolution = 300000  # Match InterMimic resolution
        dynamic_asset_options.vhacd_params.max_num_vertices_per_ch = 64  # Match InterMimic

        # Set max_convex_hulls based on object type (matching InterMimic)
        dynamic_asset_options.vhacd_params.max_convex_hulls = 10  # Default
        if obj_name in ['kettle', 'cup']:
            dynamic_asset_options.vhacd_params.max_convex_hulls = 80
        elif obj_name == 'chair':
            dynamic_asset_options.vhacd_params.max_convex_hulls = 50
    ...
    def _load_parahome_asset(self):
        ...
        for static_obj in self.cfg['env']['in_scene_obj_static']:
            self._set_static_objects(asset_root, static_obj)
    ...
    def _build_target(self, env_id, env_ptr):
        ...
        for obj in self.cfg['env']['in_scene_obj_static']: 
            ...
            if obj == 'book_no_collision':
                # Note: This uses scene_number dynamically, but frame 2460 is still hardcoded
                # If book_no_collision should only work for scene 6, consider adding a check
                with open(f'skillmimic/data/obj_transformations/s{self.scene_number}/object_transformations.pkl', 'rb') as f:
                ...
                if obj == 'sink_plane':
                    ...
                    object_transformations = open(f'skillmimic/data/obj_transformations/s{self.scene_number}/object_transformations.pkl', 'rb')
                ...
                if obj == 'diningtable_plane':
                    ...
                    # Load object transformations from the correct scene
                    object_transformations = open(f'skillmimic/data/obj_transformations/s{self.scene_number}/object_transformations.pkl', 'rb')
                    object_transformations = pickle.load(object_transformations)
                    desk_pose.p.x = object_transformations[0][f'{obj}_base'][0,3]
                    desk_pose.p.y = object_transformations[0][f'{obj}_base'][1,3]
