...
class SkillMimicParahome(HumanoidWholeBodyWithObjectParahome): 
    ...
    def _compute_reset(self):
        # Extract current and reference key body positions
        key_body_pos = self._rigid_body_pos[:, self._key_body_ids, :]  # [num_envs, num_key_bodies, 3]

        # Extract reference key body positions from _curr_ref_obs
        # Structure: root_pos(3) + root_rot(3) + dof_pos(dof_obs_size) + dof_vel(dof_obs_size) + target_states(10) + key_body_pos(num_key_bodies*3) + contact(1)
        key_body_start_idx = 6 + self._dof_obs_size*2 + 10
        key_body_end_idx = key_body_start_idx + len(self._key_body_ids)*3
        ref_key_body_pos_flat = self._curr_ref_obs[:, key_body_start_idx:key_body_end_idx]
        ref_key_body_pos = ref_key_body_pos_flat.view(-1, len(self._key_body_ids), 3)  # [num_envs, num_key_bodies, 3]

        # Extract current and reference object positions
        # Current object position from simulation
        obj_pos = self._target_states[:, 0:3]  # [num_envs, 3]

        # Reference object position from _curr_ref_obs
        # Structure: root_pos(3) + root_rot(3) + dof_pos(dof_obs_size) + dof_vel(dof_obs_size) + obj_pos(3) + ...
        obj_start_idx = 6 + self._dof_obs_size * 2
        ref_obj_pos = self._curr_ref_obs[:, obj_start_idx:obj_start_idx+3]  # [num_envs, 3]

        tracking_error_threshold = self.cfg["env"].get("tracking_error_threshold", 0.5)
        object_tracking_error_threshold = self.cfg["env"].get("object_tracking_error_threshold", 0.5)
        ...
        self.reset_buf[:], self._terminate_buf[:] = compute_humanoid_reset(self.reset_buf, self.progress_buf,
                                                   ...
                                                   key_body_pos, ref_key_body_pos, tracking_error_threshold,
                                                   obj_pos, ref_obj_pos, object_tracking_error_threshold,
                                                   self.cfg["env"]["NR"],
                                                   )
        # reweight the motion
...
def compute_humanoid_reset(reset_buf, progress_buf, contact_buf, rigid_body_pos,
                           ...
                           obj_pos, ref_obj_pos, object_tracking_error_threshold,
                           NR = False):
    # type: (Tensor, Tensor, Tensor, Tensor, float, bool, Tensor, Tensor, Tensor, Tensor, bool, int, Tensor, Tensor, float, Tensor, Tensor, float, bool) -> Tuple[Tensor, Tensor]
    terminated = torch.zeros_like(reset_buf)

    if (enable_early_termination):
        # Check 1: Falling (pelvis height too low)
        ...
        # Check 2: Human body tracking error too large (mean L2 distance across key bodies)
        # key_body_pos: [num_envs, num_key_bodies, 3]
        # ref_key_body_pos: [num_envs, num_key_bodies, 3]
        tracking_error = torch.norm(ref_key_body_pos - key_body_pos, p=2, dim=-1).mean(dim=-1)  # [num_envs]
        tracking_failed = tracking_error > tracking_error_threshold
        tracking_failed *= (progress_buf > 1)  # Skip first frame

        terminated = torch.where(tracking_failed, torch.ones_like(reset_buf), terminated)

        # Check 3: Object tracking error too large (L2 distance between object positions)
        # obj_pos: [num_envs, 3]
        # ref_obj_pos: [num_envs, 3]
        object_tracking_error = torch.norm(ref_obj_pos - obj_pos, p=2, dim=-1)  # [num_envs]
        object_tracking_failed = object_tracking_error > object_tracking_error_threshold
        object_tracking_failed *= (progress_buf > 1)  # Skip first frame

        terminated = torch.where(object_tracking_failed, torch.ones_like(reset_buf), terminated)
    ...
    if isTest and NR:
        ...
        reset = torch.where(progress_buf >= envid2episode_lengths-1, torch.ones_like(reset_buf), terminated) #ZC
    # print(f'isTest: {isTest}, NR: {NR}, enable_early_termination: {enable_early_termination}')
